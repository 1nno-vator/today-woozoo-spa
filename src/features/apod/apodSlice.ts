...
export const apodSlice = createSlice({
  ...
  reducers: {
    ADD_DAY(state) {
      let nextDate = moment(state.targetDate)
        .add(1, "days")
        .format("YYYY-MM-DD");
    ...
    SUB_DAY(state) {
      let prevDate = moment(state.targetDate)
        .subtract(1, "days")
        .format("YYYY-MM-DD");
...
export let FETCH_DATA = createAsyncThunk(
  "API/FETCH_DATA",
  async (_targetDate: string) => {
    // console.log(`http://localhost:3500/${_targetDate}`);
    return axios({
      method: "get",
      url: `${process.env.REACT_APP_API_URL}/woozooserver/${_targetDate}`,
    })
      .then((res) => {
        return res.data;
      })
